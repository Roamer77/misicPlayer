...
const Rocket = ({color}) => (<Path fill-rule="evenodd" clip-rule="evenodd"
                            ...
                            fill={color}/>);
const Note = ({color}) => (<Path opacity="1" fill-rule="evenodd" clip-rule="evenodd" y={20} x={20}
...
const Background = ({color}) => (<Svg width="62" height="62" viewBox="0 0 62 62" fill="none">
...
const ActionButton = ({isActive}) => {
    ...
    return (
        <View>
            <CircleTrackProgress style={{position: 'absolute', right: -2.4, bottom: -2.5}} tintColor={'#a89830'}
                                 width={68} progress={new Animated.Value(trackProgress * 0.000005)}/>
            <Background color={isActive?'#3E2AD1':'rgba(190,190,208,0.99)'}/>

               { flag && <Animated.View
                        style={{position: 'absolute',}}>
                        <Svg width="62" height="62">
                            <Rocket color={isActive?'white':'rgba(115,115,115,0.64)'}/>
                        </Svg>
                    </Animated.View>}
               { !flag &&  <Animated.View style={{position: 'absolute', transform: [{scale: anim.current}]}}>
                        <Svg width="62" height="62">
                            <Note color={isActive?'white':'rgba(115,115,115,0.64)'}/>
                        </Svg>
                    </Animated.View>}
